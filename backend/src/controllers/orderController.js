...
export async function createOrder(req, res, next) {
  try {
    ...
    if (customerName.length < 2 || !/^\S+@\S+\.\S+$/.test(email) || phone.length < 7 || address.length < 5) return res.status(400).json({ message: 'Valid customer name, email, phone, and delivery address are required' })
    ...
    const order = await Order.create({ customerName, email, phone, address, product: product._id, productName: product.name, unitPrice: product.price, quantity, total: Number((product.price * quantity).toFixed(2)) })
