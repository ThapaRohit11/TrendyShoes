...
export default function AuthPage({ register = false }) {
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      if (register) {
        ...
        setUser(data.user); navigate(data.user.role === 'admin' ? '/admin' : '/forbidden')
      }
  ...
  return <><Navbar/><main className="min-h-[calc(100vh-88px)] bg-slate-100 grid place-items-center p-4">
    <form onSubmit={awaitingTwoFactor ? verifyCode : submit} className="w-full max-w-md rounded-2xl bg-white p-8 shadow-xl space-y-5">
      ...
      <div><p className="text-red-500 font-semibold">{register ? 'CREATE USER ACCOUNT' : 'ACCOUNT ACCESS'}</p><h1 className="text-3xl font-bold">{register ? 'Create account' : 'Welcome back'}</h1></div>
      {success && <p className="rounded-lg bg-green-50 p-3 text-green-700">{success} <Link className="font-bold underline" to="/admin/login">Log in</Link></p>}
      {error && <p className="rounded-lg bg-red-50 p-3 text-red-700">{error}</p>}
      ...
      {!register && <div className="text-right"><Link className="text-sm font-semibold text-red-500 hover:underline" to="/admin/forgot-password">Forgot password?</Link></div>}
      ...
      <p className="text-center text-slate-600">{register ? 'Already registered?' : 'Need an admin account?'} <Link className="font-semibold text-red-500" to={register ? '/admin/login' : '/admin/register'}>{register ? 'Log in' : 'Register'}</Link></p>
      </>}
    </form>
  </main></>
}
